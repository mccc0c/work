...
});j.find(">"+d.nav+" li").on(d.event,function(){d.before();a(this).addClass("current").siblings().removeClass("current");var k=a(this).index();j.find(">"+d.content+">ul>li").eq(k).show().siblings().hide();
d.after();return false;});}function h(k){j.find(".nav li").eq(k).addClass("current").siblings().removeClass("current");j.find(".content>ul>li").eq(k).show().siblings().hide();
...
}}if(e.dialog){a(document).on("click",g,function(t){k=a(this);t.stopPropagation();e.beforeOpen(k);i();e.onOpen(k);});}a(document).on("click",r+" .dclose",function(){if(!k){return;
...
h.animate({height:e+"px"});},d.duration);});});},sidemenu:function(b){var c={event:"mouseenter"};var d=a.extend({},c,b);return this.each(function(){var f=a(this);
var e=f.find(".content").width();f.on(d.event,function(){a(this).animate({right:0},"fast");});f.on("mouseleave",function(){a(this).animate({right:-e+"px"},"fast");
});});},wordsleft:function(b){var c={};var d=a.extend({},c,b);return this.each(function(){var e=a(this);a(document).keyup(function(){e.text(110-a(d.textarea).val().length);
});});},scrollline:function(b){var c={};var d=a.extend({},c,b);return this.each(function(){var g=a(this);function e(){g.find("ul:first").animate({marginTop:"-25px"},500,function(){a(this).css({marginTop:"0px"}).find("li:first").appendTo(this);
});}var f=setInterval(function(){e();},1000);g.hover(function(){clearInterval(f);},function(){f=setInterval(function(){e();},1000);});});},inout:function(b){var c={};
var d=a.extend({},c,b);return this.each(function(){var j=a(this);var i,g;var e=j.find(".num li").length;var f=0;j.find(".content li").first().show();j.hover(function(){clearInterval(i);
},function(){i=setInterval(function(){if(f>=e-1){f=-1;}f++;h(f);},3600);}).trigger("mouseleave");j.find(".num li").hover(function(){f=j.find(".num li").index(this);
g=setTimeout(function(){h(f);},120);},function(){clearTimeout(g);});function h(k){j.find(".num li").eq(k).addClass("on").siblings().removeClass("on");j.find(".content li").eq(k).fadeIn("200").siblings().fadeOut("200");
...
for(var d=0;d<a.length;d++){var b=a[d].split("=");if(b[0]==c){return b[1];}}return"";}function removeCookie(a){setCookie(a,1,-1);}function urlArgs(d){var c=location.search.substring(1).split("&");
