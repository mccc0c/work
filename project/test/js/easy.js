(function(a){a.fn.extend({tab:function(b){var c={event:"mouseenter",auto:false,showDuration:0,duration:3600,nav:".nav",content:".content",before:function(){},after:function(){}};
...
});j.find(">"+d.nav+" li").on(d.event,function(){k=a(this).index();d.before(k,j);if(d.stop){return;}a(this).addClass("current").siblings().removeClass("current");
var k=a(this).index();j.find(">"+d.content+">ul>li").eq(k).show(d.showDuration,function(){d.after(k,j);}).siblings().hide();return false;});}function h(k){j.find(".nav li").eq(k).addClass("current").siblings().removeClass("current");
j.find(".content>ul>li").eq(k).show().siblings().hide();return false;}});},tmpl:function(c,b){(function(){var e={};this.tmpl=function d(h,g){var f=!/\W/.test(h)?e[h]=e[h]||d(document.getElementById(h).innerHTML):new Function("obj","var p=[],print=function(){p.push.apply(p,arguments);};with(obj){p.push('"+h.replace(/[\r\t\n]/g," ").split("<%").join("\t").replace(/((^|%>)[^\t]*)'/g,"$1\r").replace(/\t=(.*?)%>/g,"',$1,'").split("\t").join("');").split("%>").join("p.push('").split("\r").join("\\'")+"');}return p.join('');");
...
var l=m.width();var p=false;var n,j;var o=window.innerWidth-a("body").width();d.canopen=true;if(e.mobileDetect&&d.isMobile()){m.addClass("dmobile");}if(!m.next(".modalbg")[0]){m.after('<div class="modalbg"></div>');
...
e.beforeOpen(k);if(d.canopen){i();}e.onOpen(k);});}else{k=true;}a(document).on("click",r+" .dclose",function(){if(!k){return;}s();e.onClose();k=null;return false;
});a(document).on("click",r+" .dcancle",function(){if(!k){return;}s();e.onClose();k=null;return false;});a(document).on("click",r+" .dokay",function(){if(!k){return;
...
}if(d.showall){g.addClass(e);f.show();}a(document).on(d.event,g,function(k){var m=a(k.target),l=m.next(d.content),j=m.siblings(),i=m.siblings().next(d.content);
if(d.animate){if(m.is("."+e)){l.slideUp();setTimeout(function(){m.removeClass(e);},400);}else{l.slideDown();setTimeout(function(){m.addClass(e);},400);
}if(d.collapse){i.slideUp();setTimeout(function(){j.removeClass(e);},400);}}else{if(m.is("."+e)){m.removeClass(e);l.hide();}else{m.addClass(e);l.show();
